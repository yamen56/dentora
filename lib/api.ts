...
export async function loadOwnedFlashcard(flashcardId: string) {
  ...
  if (flashcard.course.instructorId !== user.id && user.role !== "ADMIN") {
    return { error: json({ error: "forbidden" }, 403) } as const;
  }
  return { user, flashcard } as const;
}

export async function loadOwnedEnrollment(enrollmentId: string) {
  const user = await getApiUser();
  if (!user) return { error: json({ error: "unauthorized" }, 401) } as const;
  ...
  if (!enrollment) return { error: json({ error: "notFound" }, 404) } as const;

  if (enrollment.course.instructorId !== user.id && user.role !== "ADMIN") {
    return { error: json({ error: "forbidden" }, 403) } as const;
  }
  return { user, enrollment } as const;
}
