...
export default async function CourseDetailPage({
  ...
  // Free-preview lectures ship their flashcards + questions so visitors get
  // the full lecture experience from the course page (graded locally).
  const previewIds = course.lectures
    .filter((l) => l.isPreview)
    .map((l) => l.id);
  const [previewFlashcards, previewQuestions] = previewIds.length
    ? await Promise.all([
        prisma.flashcard
          .findMany({
            where: { lectureId: { in: previewIds } },
            orderBy: { order: "asc" },
            select: {
              id: true,
              front: true,
    ...
    previewIds.map((id) => [
      id,
      {
        flashcards: previewFlashcards
          .filter((f) => f.lectureId === id)
          .map((f) => ({ id: f.id, front: f.front, back: f.back, hint: f.hint })),
        questions: previewQuestions
          .filter((q) => q.lectureId === id)
          .map((q) => ({
            id: q.id,
            type: q.type,
  ...
  const curriculumLectures = course.lectures.map((l) => ({
    id: l.id,
    title: l.title,
    duration: l.duration,
    isPreview: l.isPreview,
    hasPdf: Boolean(l.pdfUrl),
    hasVideo: Boolean(l.videoUrl),
    examPeriod: l.examPeriod,
    open: enrolled || l.isPreview,
    showPreviewBadge: l.isPreview && !enrolled,
  }));
